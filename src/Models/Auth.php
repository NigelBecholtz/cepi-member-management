...
class Auth {
    ...
    public function getAll() {
        $stmt = $this->db->query("
            ...
            ORDER BY o.organisation_name, a.username
        ");
    ...
    public function getById($authId) {
        $stmt = $this->db->prepare("
            SELECT a.*, o.organisation_name
            FROM organisation_auth a
            JOIN organisations o ON a.organisation_id = o.organisation_id
            WHERE a.auth_id = :auth_id AND a.is_active = TRUE
        ");
    ...
    public function update($authId, $username, $email = null, $password = null) {
        $updates = [];
        ...
        if ($email !== null) {
            ...
            $updates[] = "email_lookup_hash = :email_lookup_hash";
            $params[':email_hash'] = $emailHash;
            $params[':email_lookup_hash'] = $emailLookupHash;
        }
    ...
    public function delete($authId) {
        ...
        return $stmt->execute([':auth_id' => $authId]);
    }
}
